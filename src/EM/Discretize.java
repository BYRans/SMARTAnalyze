 ...
 *
 * -F <br>
 * Use equal-frequency instead of equal-width discretization if class-based
 * discretisation is turned off.
 * <p>
 *
 * -O <br>
 * Optimize the number of bins using a leave-one-out estimate of the entropy
 * (for equal-width binning). If this is set then the -B option will be ignored.
 * <p>
 *
 * -R col1,col2-col4,... <br>
...
public class Discretize extends PotentialClassIgnorer implements
		UnsupervisedFilter, OptionHandler, WeightedInstancesHandler {
	...
	public Enumeration listOptions() {
		...
		newVector.addElement(new Option(
				"tSpecifies the (maximum) number of bins to divide numeric"
						+ " attributes into.n" + "t(default = 10)", "B", 1,
				"-B <num>"));
		newVector
				.addElement(new Option(
						"tSpecifies the desired weight of instances per bin forn"
								+ "tequal-frequency binning. If this is set to a positiven"
								+ "tnumber then the -B option will be ignored.n"
								+ "t(default = -1)", "M", 1, "-M <num>"));
		newVector.addElement(new Option(
				"tUse equal-frequency instead of equal-width discretization.",
				"F", 0, "-F"));
		newVector
				.addElement(new Option(
						"tOptimize number of bins using leave-one-out estimaten"
								+ "tof estimated entropy (for equal-width discretization).n"
								+ "tIf this is set then the -B option will be ignored.",
						"O", 0, "-O"));
		newVector.addElement(new Option(
				"tSpecifies list of columns to Discretize. First"
						+ " and last are valid indexes.n"
						+ "t(default: first-last)", "R", 1,
				"-R <col1,col2-col4,...>"));
		newVector.addElement(new Option(
				"tInvert matching sense of column indexes.", "V", 0, "-V"));
		newVector.addElement(new Option(
				"tOutput binary attributes for discretized attributes.", "D",
				0, "-D"));
	 ...
	 * -O <br>
	 * Optimize the number of bins using a leave-one-out estimate of the entropy
	 * (for equal-width binning). If this is set then the -B option will be
	 * ignored.
	 * <p>
	 *
	 * -R col1,col2-col4,... <br>
	...
	public void setOptions(String[] options) throws Exception {
		...
		if (weight.length() != 0) {
			setDesiredWeightOfInstancesPerInterval((new Double(weight))
					.doubleValue());
		} else {
	 ...
	 *
	 * @param attributes
	 *            an array containing indexes of attributes to Discretize. Since
	 ...
	 *
	 * @param the
	 *            index (from 0) of the attribute to get the cut points of
	...
	protected void findNumBins(int index) {
		double min = Double.MAX_VALUE, max = -Double.MIN_VALUE, binWidth = 0, entropy, bestEntropy = Double.MAX_VALUE, currentVal;
		double[] distribution;
		int bestNumBins = 1;
		Instance currentInstance;
		...
		for (int i = 0; i < m_NumBins; i++) {
			...
			for (int k = 0; k < i + 1; k++) {
				...
				entropy -= distribution[k]
						* Math.log((distribution[k] - 1) / binWidth);
			}
			// Best entropy so far?
					...
					if (m_CutPoints[i] == null) {
						...
						for (int j = 0; j < m_CutPoints[i].length; j++) {
							...
							attributes.addElement(new Attribute(
									getInputFormat().attribute(i).name(),
									attribValues));
						}
					}
				}
	...
	protected void convertInstance(Instance instance) {
		...
		copyStringValues(inst, false, instance.dataset(),
				getInputStringIndex(), getOutputFormat(),
